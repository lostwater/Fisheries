...
function initAvatarUpload() {
    ...
    //$("#avatar-crop-box p").removeClass("hidden");
...
function initAvatarCrop(img) {
    img.Jcrop({
        ...
        jcrop_api.setOptions({ aspectRatio: radio });
...
function saveAvatar(saveType) {
    var img = $("#preview-pane .preview-container img");
    $("#avatar-crop-box button").addClass("disabled");
    $("#avatar-crop-box p").addClass("hidden");
    //$("#avatar-crop-box").addClass("hidden");
    $.ajax({
        type: "POST",
        url: "/ImageUploader/Save",
        traditional: true,
        data: {
            w: img.css("width"),
            h: img.css("height"),
        ...
        if (data.success === true) {
            ...
            $("#avatar-result img").attr("src", data.avatarFileLocation + "?time=" + new Date());

            $("#avatar-result").removeClass("hidden");
            $("#avatar-result input").attr("value", data.avatarFileLocation);
            $("#avatar-result input").addClass("hidden");
            if (!keepCropBox) {
                $("#avatar-crop-box").addClass("hidden");
            }
